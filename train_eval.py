...
class Trainner():
    def __init__(self, model_type, train_dataset, eval_dataset, num_epoch, batch_size):
        ...
        tr_data = tf.data.TFRecordDataset(train_dataset).map(data_parser).shuffle(1000).batch(batch_size)
        val_data = tf.data.TFRecordDataset(eval_dataset).map(data_parser).batch(batch_size)
